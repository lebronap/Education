...
public class SharedPreUtils {
    ...
    public static void saveObject(Context context, Object obj){
        try {
            ...
            //保存该16进制数组
            sharedata.putString(USER_INFO, bytesToHexString);
            sharedata.commit();
        } catch (IOException e) {
            e.printStackTrace();
            Log.e(TAG, "保存obj失败");
    ...
    public static Object readObject(Context context){
        try {
            SharedPreferences sharedata = context.getSharedPreferences(FILENAME, 0);
            if (sharedata.contains(USER_INFO)) {
                String string = sharedata.getString(USER_INFO, "");
